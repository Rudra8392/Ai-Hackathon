import streamlit as st import pandas as pd import plotly.graph_objects as go from datetime import datetime, timedelta import numpy as np import joblib

def load_data(): # Load preprocessed data data = pd.read_csv('preprocessed_data.csv') data['Time'] = pd.to_datetime(data['Time'])
...
return data
def create_monitoring_dashboard(): st.set_page_config(page_title="Oil & Gas Production Monitoring", layout="wide") st.title("Oil & Gas Production Monitoring Dashboard")
...
def main(): create_monitoring_dashboard()
